...
type MorphologySearchItem = { entry: MorphologyEntry; fields: string[] }
type TopographySearchItem = { entry: TopographyEntry; fields: string[] }

const catalog = shallowRef<Catalog | null>(null)
const morphologyIndex = shallowRef<MorphologySearchItem[]>([])
const standardTopographyIndex = shallowRef<TopographySearchItem[]>([])
const optionalTopographyIndex = shallowRef<TopographySearchItem[]>([])
const loading = ref(true)
const loadError = ref('')
const morphologyQuery = ref('')
const topographyQuery = ref('')
const deferredMorphologyQuery = ref('')
const deferredTopographyQuery = ref('')
...
const morphologyInputRef = ref<HTMLInputElement | null>(null)
const copied = ref(false)
let morphologyTimer: ReturnType<typeof window.setTimeout> | undefined
let topographyTimer: ReturnType<typeof window.setTimeout> | undefined
...
const topographySearchScore = (fields: string[], query: string) => {
  const normalizedQuery = normalizePathologySearch(query)
  if (!normalizedQuery) return 0

  const normalizedFields = fields.map(normalizePathologySearch).filter(Boolean)
  if (normalizedFields.some(field => field.includes(normalizedQuery))) return 160

  const words = normalizedFields.flatMap(field => field.split(/[^a-z0-9+/-]+/).filter(Boolean))
  const usedIndexes = new Set<number>()
  let score = 0

  for (const token of normalizedQuery.split(/\s+/).filter(Boolean)) {
    let index = words.findIndex((word, wordIndex) => !usedIndexes.has(wordIndex) && word === token)
    if (index >= 0) score += 50
    else {
      index = words.findIndex((word, wordIndex) => !usedIndexes.has(wordIndex) && token.length >= 2 && word.startsWith(token))
      if (index < 0) return -1
      score += 30
    }
...
const matchingMorphologyResults = computed(() => {
  ...
  return morphologyIndex.value
    .filter(({ entry }) => behaviourFilter.value === 'all' || entry.behaviour === behaviourFilter.value)
    .map(({ entry, fields }) => {
      const exactCode = codeQueryMatches(entry, query)
      return { entry, score: exactCode ? 1000 : pathologySearchScore(fields, query) }
    })
...
const allTopographyIndex = computed(() => {
  return includeOptionalTopography.value
    ? [...standardTopographyIndex.value, ...optionalTopographyIndex.value]
    : standardTopographyIndex.value
})
...
const topographyResults = computed(() => {
  const query = deferredTopographyQuery.value.trim()
  if (!query) {
    return FEATURED_SITES
      .map(code => allTopographyIndex.value.find(item => item.entry.code === code)?.entry)
      .filter(Boolean)
      .slice(0, topographyLimit.value) as TopographyEntry[]
  }
  const normalized = normalizePathologySearch(query)
  return allTopographyIndex.value
    .map(({ entry, fields }) => {
      const exactCode = normalizePathologySearch(entry.code).includes(normalized)
      return { entry, score: exactCode ? 1000 : topographySearchScore(fields, query) }
    })
    .filter(item => item.score >= 0)
    .sort((a, b) => b.score - a.score)
    .map(item => item.entry)
...
const morphologyAliases = computed(() => pathologyAliasExpansions(deferredMorphologyQuery.value))
...
const toggleTopography = (entry: TopographyEntry) => {
  const isSelected = selectedTopography.value?.code === entry.code
    && selectedTopography.value?.optional === entry.optional
  selectedTopography.value = isSelected ? null : entry
}
const clearTopography = () => {
  selectedTopography.value = null
  topographyQuery.value = ''
  deferredTopographyQuery.value = ''
}
const clearMorphology = () => {
  selectedMorphology.value = null
}
const clearSelections = () => {
  clearTopography()
  clearMorphology()
}
const continueToMorphology = () => {
  morphologyInputRef.value?.scrollIntoView({ block: 'center' })
  window.requestAnimationFrame(() => morphologyInputRef.value?.focus({ preventScroll: true }))
}
...
watch(morphologyQuery, (value) => {
  if (morphologyTimer) window.clearTimeout(morphologyTimer)
  morphologyLimit.value = 36
  if (!value.trim()) {
    deferredMorphologyQuery.value = ''
    return
  }
  morphologyTimer = window.setTimeout(() => {
    deferredMorphologyQuery.value = value
  }, 160)
})

watch(topographyQuery, (value) => {
  if (topographyTimer) window.clearTimeout(topographyTimer)
  topographyLimit.value = 24
  if (!value.trim()) {
    deferredTopographyQuery.value = ''
    return
  }
...
onMounted(async () => {
  try {
    ...
    const loadedCatalog = await response.json() as Catalog
    catalog.value = loadedCatalog
    morphologyIndex.value = loadedCatalog.morphology.map(entry => ({ entry, fields: morphologyFields(entry) }))
    standardTopographyIndex.value = loadedCatalog.topography.map(entry => ({ entry, fields: topographyFields(entry) }))
    optionalTopographyIndex.value = loadedCatalog.optionalTopography.map(entry => ({ entry, fields: topographyFields(entry) }))
  } catch (error: any) {
        ...
        <section class="coding-strip" aria-labelledby="coding-summary-title">
          <div class="strip-heading">
            ...
            <div class="summary-actions">
              <button v-if="selectedTopography || selectedMorphology" class="clear-selection-button" @click="clearSelections">
                <v-icon size="17">mdi-backspace-outline</v-icon><span>Bỏ chọn tất cả</span>
              </button>
          ...
          <div class="selection-grid">
            <div class="selection-cell" :class="{ empty: !selectedTopography }">
              <div class="selection-label"><span>01 · VỊ TRÍ CƠ QUAN / TOPOGRAPHY</span><button v-if="selectedTopography" title="Bỏ chọn vị trí" @click="clearTopography"><v-icon size="17">mdi-close</v-icon></button></div>
            ...
            <div class="selection-cell" :class="{ empty: !selectedMorphology }">
              <div class="selection-label"><span>02 · HÌNH THÁI / MORPHOLOGY</span><button v-if="selectedMorphology" title="Bỏ chọn hình thái" @click="clearMorphology"><v-icon size="17">mdi-close</v-icon></button></div>
          ...
          <section v-if="selectedMorphology" class="selected-profile" aria-labelledby="selected-profile-title">
            <div class="detail-title">
              <div><p>HỒ SƠ MÃ ĐANG CHỌN</p><h2 id="selected-profile-title">{{ selectedMorphology.code }} · {{ translatedText(selectedMorphology.preferred) || selectedMorphology.preferred }}</h2></div>
              <button class="icon-button" title="Đóng hồ sơ" @click="clearMorphology"><v-icon size="20">mdi-close</v-icon></button>
            </div>
            ...
            <details v-if="selectedMorphology.terms.length" class="term-disclosure">
              <summary><span>Thuật ngữ và tên gọi liên quan</span><strong>{{ selectedMorphology.terms.length }}</strong><v-icon size="18">mdi-chevron-down</v-icon></summary>
        ...
        <section class="lookup-grid">
          <div class="morphology-panel">
            <div class="panel-heading">
              <div><p>BƯỚC 2 · CHẨN ĐOÁN / MORPHOLOGY</p><h2>Tra mã hình thái</h2></div>
            ...
            <div class="search-row">
              <label class="search-box">
                ...
                <input ref="morphologyInputRef" v-model="morphologyQuery" type="search" placeholder="Tên Việt, English, BCC, SCC, HCC, PTC, 8140/3...">
          ...
          <aside class="topography-panel">
            <div class="panel-heading compact">
              <div><p>BƯỚC 1 · CƠ QUAN / TOPOGRAPHY</p><h2>Chọn vị trí u nguyên phát</h2></div>
              <button v-if="selectedTopography" class="clear-selection-button" @click="clearTopography"><v-icon size="17">mdi-close</v-icon><span>Bỏ chọn</span></button>
            </div>
            <div v-if="selectedTopography" class="selected-site-summary">
              <span class="site-code">{{ selectedTopography.code }}</span>
              <span><strong>{{ translatedText(selectedTopography.preferred) || sourceOnlyLabel }}</strong><small>{{ selectedTopography.preferred }}</small></span>
              <button title="Bỏ chọn vị trí" @click="clearTopography"><v-icon size="18">mdi-close</v-icon></button>
            </div>
            <div v-if="selectedTopography" class="topography-feedback" aria-live="polite">
              <div>
                <span>{{ selectedMorphology ? 'ĐÃ ĐỦ HAI TRỤC MÃ HÓA' : 'ĐÃ GHI NHẬN VỊ TRÍ' }}</span>
                <strong>{{ selectedMorphology ? combinedCode : `${selectedTopography.code} · Chờ mã hình thái` }}</strong>
                <p v-if="!selectedMorphology">Vị trí không tự lọc chẩn đoán vì ICD-O mã hóa vị trí và hình thái trên hai trục độc lập. Tiếp tục chọn hình thái để tạo mã hồ sơ.</p>
                <p v-else>Mã vị trí đã được ghép với mã hình thái và hành vi đang chọn.</p>
              </div>
              <button v-if="!selectedMorphology" class="next-step-button" @click="continueToMorphology">
                Chọn hình thái <v-icon size="18">mdi-arrow-up</v-icon>
              </button>
              <button v-else class="next-step-button" @click="copyCombinedCode">
                {{ copied ? 'Đã sao chép' : 'Sao chép mã' }} <v-icon size="18">{{ copied ? 'mdi-check' : 'mdi-content-copy' }}</v-icon>
              </button>
            </div>
            <label class="search-box topo-search">
              <v-icon size="20">mdi-map-marker-search-outline</v-icon>
              <input v-model="topographyQuery" type="search" placeholder="Vú, phổi, dạ dày, breast, C50...">
            ...
            <div class="topography-list">
              <button v-for="entry in topographyResults" :key="`${entry.optional}-${entry.code}`" class="topography-item" :class="{ selected: selectedTopography?.code === entry.code && selectedTopography?.optional === entry.optional }" @click="toggleTopography(entry)">
                ...
                <v-icon size="18">{{ selectedTopography?.code === entry.code && selectedTopography?.optional === entry.optional ? 'mdi-check-circle' : 'mdi-chevron-right' }}</v-icon>
              </button>
            </div>
...
.coding-strip,.morphology-panel,.topography-panel,.source-panel { background: #fff; border: 1px solid #cad9df; border-radius: 6px; box-shadow: 0 12px 30px rgba(17,48,65,.07); }.coding-strip { padding: 22px; }.strip-heading,.panel-heading,.detail-title { display: flex; align-items: flex-end; justify-content: space-between; gap: 20px; }.strip-heading h2,.panel-heading h2,.detail-title h2,.source-panel h2 { margin: 0; font: 700 1.6rem/1.2 var(--font-heading); }.summary-actions { display: flex; align-items: center; gap: 8px; }.icon-button { width: 38px; height: 38px; flex: 0 0 38px; display: grid; place-items: center; color: #126b75; background: #f3f8f9; border: 1px solid #c7d9df; border-radius: 4px; }.icon-button:disabled { opacity: .45; cursor: not-allowed; }.clear-selection-button { min-height: 38px; padding: 0 10px; display: inline-flex; align-items: center; justify-content: center; gap: 5px; color: #7b3e47; background: #fff7f7; border: 1px solid #e5c5ca; border-radius: 4px; font-size: .67rem; font-weight: 800; }.clear-selection-button:hover { color: #9b2434; border-color: #cf8994; background: #fff0f1; }
.selection-grid { margin-top: 18px; display: grid; grid-template-columns: 1fr 1fr .8fr; border: 1px solid #cadbe1; }.selection-cell,.combined-cell { min-height: 132px; padding: 17px; display: flex; flex-direction: column; text-align: left; background: #f7fafb; border-right: 1px solid #cadbe1; }.selection-label { display: flex; align-items: center; justify-content: space-between; gap: 10px; }.selection-label > span,.combined-cell > span { color: #17877f; font-size: .61rem; font-weight: 900; }.selection-label button { width: 27px; height: 27px; flex: 0 0 27px; display: grid; place-items: center; color: #78505a; background: #fff; border: 1px solid #dccbd0; border-radius: 3px; }.selection-cell strong,.combined-cell strong { margin-top: 8px; color: #123448; font-size: 1.05rem; }.selection-cell p,.combined-cell p { margin: 8px 0 0; color: #425f70; font-size: .73rem; }.selection-cell small { margin-top: 3px; color: #748995; font-size: .66rem; }.selection-cell.empty { border-top: 3px solid #9eafb8; }.combined-cell { border-right: 0; background: #102f43; }.combined-cell > span { color: #63ddd7; }.combined-cell strong { color: #fff; font: 700 1.3rem var(--font-heading); }.combined-cell p { color: #bdd0da; }.combined-cell.ready { border-top: 3px solid #f0cb58; }.coding-note { margin: 14px 0 0; display: flex; align-items: flex-start; gap: 8px; color: #667e8b; font-size: .7rem; }
.selected-profile { margin-top: 18px; padding-top: 18px; border-top: 1px solid #d7e2e6; }.selected-profile .detail-title { align-items: center; }.selected-profile .detail-title h2 { font-size: 1.35rem; }.term-disclosure { margin-top: 14px; border: 1px solid #d3dfe4; }.term-disclosure summary { min-height: 44px; padding: 0 12px; display: flex; align-items: center; gap: 9px; color: #31586a; background: #f4f8f9; cursor: pointer; list-style: none; font-size: .7rem; font-weight: 800; }.term-disclosure summary::-webkit-details-marker { display: none; }.term-disclosure summary strong { margin-left: auto; padding: 3px 7px; color: #106f74; background: #dff3f1; border-radius: 10px; font-size: .6rem; }.term-disclosure[open] summary .v-icon { transform: rotate(180deg); }.term-disclosure .term-table { margin-top: 0; border: 0; border-top: 1px solid #d3dfe4; }
...
@media (max-width: 1100px) { .icdo-hero { grid-template-columns: 1fr; }.hero-stats { max-width: 660px; }.lookup-grid { grid-template-columns: 1fr; }.topography-panel { order: -1; }.selection-grid { grid-template-columns: 1fr 1fr; }.combined-cell { grid-column: 1 / -1; min-height: 98px; }.selection-cell:nth-child(2) { border-right: 0; }.source-panel { grid-template-columns: 1fr; } }
@media (max-width: 700px) { .icdo-hero { min-height: 0; padding: 36px 18px; gap: 26px; }.hero-copy h1 { font-size: 2.5rem; }.hero-stats { grid-template-columns: 1fr; }.hero-stats div { min-height: 78px; border-right: 0; border-bottom: 1px solid rgba(255,255,255,.16); }.icdo-workspace { width: calc(100% - 24px); padding-top: 12px; }.coding-strip,.morphology-panel,.topography-panel { padding: 14px; }.selection-grid,.search-row,.morphology-list,.detail-grid,.source-links { grid-template-columns: 1fr; }.selection-cell { border-right: 0; border-bottom: 1px solid #cadbe1; }.selection-cell:nth-child(2) { border-bottom: 1px solid #cadbe1; }.strip-heading,.panel-heading,.detail-title { align-items: flex-start; }.summary-actions { align-items: flex-end; flex-direction: column-reverse; }.clear-selection-button span { display: none; }.panel-heading > span { display: none; }.selected-profile .detail-title h2 { font-size: 1.1rem; }.table-head,.table-row { grid-template-columns: 88px 1fr; }.table-head span:last-child,.table-row span:last-child { grid-column: 1 / -1; border-top: 1px solid #d5e1e5; }.source-panel { padding: 18px; } }
</style>
