...
const performSearch = async () => {
    if (!keyword.value && !selectedOrgan.value && !selectedDiagnosis.value && !selectedTag.value) {
        rows.value = []
    ...
    try {
        rows.value = await search({
            ...
            diagnosisIds: selectedDiagnosis.value ? [selectedDiagnosis.value] : undefined,
...
const featuredCategories = [
    { icon: 'mdi-heart-pulse', title: 'Tim mạch', description: 'Bệnh lý tim và mạch máu', count: 45, color: '#e74c3c' },
    { icon: 'mdi-lungs', title: 'Hô hấp', description: 'Phổi và đường hô hấp', count: 38, color: '#3498db' },
    { icon: 'mdi-stomach', title: 'Tiêu hóa', description: 'Dạ dày, ruột, gan, mật', count: 67, color: '#27ae60' },
    { icon: 'mdi-brain', title: 'Thần kinh', description: 'Não và hệ thần kinh', count: 29, color: '#9b59b6' },
    { icon: 'mdi-bone', title: 'Cơ xương khớp', description: 'Xương, khớp và cơ', count: 52, color: '#f39c12' },
    { icon: 'mdi-water', title: 'Tiết niệu', description: 'Thận và đường tiết niệu', count: 41, color: '#1abc9c' },
]

// Recent/Featured specimens
const featuredSpecimens = [
    { id: 'SP001', title: 'Carcinoma tế bào vảy phổi', organ: 'Phổi', views: 1250 },
    { id: 'SP002', title: 'Adenocarcinoma đại tràng', organ: 'Đại tràng', views: 980 },
        ...
        <div class="main-content">
                    ...
                    <v-row align="center" class="hero-row">
                        ...
                        <v-col cols="12" lg="6" class="hero-content">
                            ...
                            <div class="trust-badges mb-6 animate-fade-in">
                                <div class="trust-badge" v-for="(t, i) in trustIndicators" :key="i">
                                    <v-icon size="14" color="accent" class="mr-1">{{ t.icon }}</v-icon>
                                    <span>{{ t.label }}</span>
                                </div>
                            </div>
                            ...
                            <p class="hero-description animate-slide-up delay-1">
                                Khám phá hơn <strong>500+</strong> mẫu bệnh phẩm vi thể được phân loại
                                theo hệ thống cơ quan. Công cụ tra cứu chuyên nghiệp dành cho
                                bác sĩ, sinh viên Y khoa và nhà nghiên cứu.
                            </p>
                            ...
                            <div class="quick-search animate-slide-up delay-2">
                                ...
                                <div class="popular-searches mt-4">
                                    ...
                                    <v-chip size="small" variant="tonal" color="white" class="mx-1">Ung thư
                                        phổi</v-chip>
                                    <v-chip size="small" variant="tonal" color="white" class="mx-1">Viêm gan</v-chip>
                                    <v-chip size="small" variant="tonal" color="white" class="mx-1">U lympho</v-chip>
                                </div>
                            </div>
                        ...
                        <v-col cols="12" lg="6" class="hero-visual d-none d-lg-block">
                            <div class="featured-grid">
                                <div v-for="(specimen, idx) in featuredSpecimens" :key="specimen.id"
                                    ...
                                    <v-icon size="20" color="white" class="arrow-icon">mdi-chevron-right</v-icon>
                                </div>
                            </div>
                        </v-col>
                    </v-row>
                ...
                <v-container>
                    <div class="section-header">
                        <div>
                            ...
                            <p class="section-subtitle">Chọn danh mục để xem các mẫu bệnh phẩm liên quan</p>
                        </div>
                        <v-btn variant="text" color="primary" append-icon="mdi-arrow-right" class="d-none d-sm-flex">
                            Xem tất cả
                        </v-btn>
                    </div>

                    <v-row class="category-grid">
                        <v-col v-for="(cat, index) in featuredCategories" :key="index" cols="6" sm="4" md="2">
                            <div class="category-card" :class="{ active: activeCategory === index }"
                                @mouseenter="activeCategory = index" @mouseleave="activeCategory = null">
                        ...
                        <v-card-text class="pa-6 pa-md-8">
                            <div class="d-flex align-center mb-6">
                                ...
                                <div class="ml-4">
                                    ...
                                    <div class="text-caption text-grey">Lọc theo nhiều tiêu chí để tìm chính xác mẫu
                                        bệnh phẩm bạn
                                        cần</div>
                                </div>
                            </div>
                            ...
                            <v-row>
                                <v-col cols="12" md="3">
                                    <v-text-field v-model="keyword" label="Từ khóa" placeholder="Nhập từ khóa..."
                            ...
                            <div v-if="hasActiveFilters" class="mt-4 d-flex align-center">
                                <v-chip color="error" variant="tonal" size="small" @click="clearFilters"
            ...
            <section v-if="showResults" id="results" class="results-section">
                <v-container>
                    ...
                    <div v-if="!loading && rows.length === 0" class="empty-state text-center py-12">
                        <v-icon size="80" color="grey-lighten-2" class="mb-4">mdi-folder-search</v-icon>
                        <h3 class="text-h6 text-grey">Không tìm thấy kết quả phù hợp</h3>
                        <p class="text-body-2 text-grey-darken-1">Hãy thử với từ khóa khác hoặc mở rộng bộ lọc</p>
                    </div>
                </v-container>
            </section>
                        ...
                        <v-row align="center" no-gutters>
                            <v-col cols="12" md="8" class="pa-8 pa-md-12">
                                ...
                                <div class="cta-actions">
                                    <v-btn color="white" variant="flat" rounded="pill" size="large" class="mr-3">
                                        <v-icon start>mdi-account-plus</v-icon>
                                        Đăng ký miễn phí
                                    </v-btn>
                                    <v-btn color="white" variant="outlined" rounded="pill" size="large">
                                        Tìm hiểu thêm
                                    </v-btn>
                                </div>
                            </v-col>
                    ...
                    <v-row>
                        <v-col cols="12" md="4">
                            <div class="footer-brand">
                                ...
                                <p class="footer-desc">
                                    Thư viện mô bệnh học số hàng đầu Việt Nam.
                                    Công cụ tra cứu chuyên nghiệp cho cộng đồng Y học.
                                </p>
                            </div>
                        </v-col>
                        <v-col cols="6" md="2">
                        ...
                        <v-col cols="12" md="4">
                            <div class="footer-links">
                                ...
                                <div class="social-links">
                                    <v-btn icon variant="tonal" color="white" size="small" class="mr-2">
...
.gradient-overlay {
    ...
    background:
        ...
        radial-gradient(circle at 80% 20%, rgba(45, 74, 62, 0.2) 0%, transparent 50%);
    pointer-events: none;
    z-index: 0;
}
