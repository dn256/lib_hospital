...
const onSave = async () => {
    ...
    try {
        ...
        // Update tags
        const { error: delError } = await supabase.from('case_version_tags').delete().eq('version_id', dbId)
        if (delError) throw delError

        if (selectedTagIds.value.length > 0) {
            const tagRows = selectedTagIds.value.map(tid => ({ version_id: dbId, tag_id: tid }))
        ...
        <div v-else class="detail-card">
            <div class="actions-header">
                ...
                <div v-if="isAdmin || canCreateCase" class="edit-actions">
                    <button v-if="!isEditing" @click="toggleEdit" class="btn btn-edit">Chỉnh sửa</button>
            ...
            <template v-else>
                <!-- EDIT VIEW -->
                <h1 class="page-title">Biên tập: {{ caseDetail.id?.substring(0, 8) }}...</h1>
                ...
                <div class="form-layout">
                    <div class="main-column">
                        <div class="form-group">
                            ...
                            <textarea v-model="caseDetail.microscopic_description" rows="15" class="input-control"></textarea>
                        </div>
                        <div class="form-group">
                            <label>Ghi chú</label>
                            <textarea v-model="caseDetail.note" rows="3" class="input-control"></textarea>
                        </div>
                    </div>
                    ...
                    <div class="side-column">
                        <div class="panel">
                            <div class="form-group">
                                <label>Cơ quan</label>
                                <v-autocomplete v-model="caseDetail.organ_id" :items="organs" item-title="name" item-value="id"
                            ...
                            <div class="form-group">
                                ...
                                <v-combobox v-model="caseDetail.diagnosis_id" :items="diagnoses" item-title="name"
                                    item-value="id" placeholder="Chọn hoặc nhập một chẩn đoán mới" variant="outlined" density="comfortable"
